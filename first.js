...
function createBlog(blogData) {
    axios.post('http://localhost:3000/admin/add-product',blogData)
        .then(response => {
            console.log('crated dtaa from server',response.data);
...
function updateBlog(id, updatedData) {
    axios.put(`http://localhost:3000/admin/edit-product`, updatedData)
        .then(() => {
...
function editBlog(id) {
    axios.get(`http://localhost:3000/admin/delete-product`,id)
        .then(response => {
            const blog = response.data;
            document.getElementById('name').value = blog.name;
            document.getElementById('phone_number').value = blog.phone_number;
...
function deleteBlog(id) {
    ...
    axios.delete('http://localhost:3000/admin/delete-product',id)
        .then(() => {
